...
* Added the ability to allow developers choose between `NavigationMapView.userCourseView` and map SDK location layer during turn-by-turn navigation. By giving value to `NavigationMapView.customizedPuck3DConfiguration` or `NavigationMapView.customizedPuck2DConfiguration`, developers could use the location puck from `MapboxMaps` during turn-by-turn navigation. To switch back to `NavigationMapView.userCourseView`, set the `NavigationMapView.customizedPuck3DConfiguration` or `NavigationMapView.customizedPuck2DConfiguration` to `nil`. ([#2968](https://github.com/mapbox/mapbox-navigation-ios/pull/2968))
